	...
	function openTaskList() {
		// click kt_drawer_example_permanent_toggler
		$('#kt_drawer_example_permanent_toggler').click();
	}
	<?php
    ...
    for (var i = 0; i < tasklists.length; i++) {
		...
		if (document.getElementsByName(tasklists[i].slug + "_" + tasklists[i].type).length > 0) {
			continue;
		}
        ...
        checkbox2.checked = true; // Sesuaikan sesuai kebutuhan
	...
	function ultrapm_addToTaskList(Name, Slug, DownloadLink, type) {
		...
		jQuery.ajax({
            ...
            success: function(data) {
				...
				// open task list kt_drawer_example_permanent_toggle
				$('#kt_drawer_example_permanent_toggle').click();
				// refresh table
				refreshTable();
            },
            error: function() {
	...
	function ultrapm_insact() {
    	...
    	Promise.all(promises).then(function () {
			...
			if(Tslugs.length > 0) {
				// do activate each Tslugs
				for (var i = 0; i < Tslugs.length; i++) {
					var slug = Tslugs[i];
					var data = {
						'action': 'ultrapm_activate_theme_bySlug',
						'slug': slug,
        	...
        	// All requests are complete, reset the button
        	button.innerHTML = 'Install and Activate';
        	button.disabled = false;
			window.location.href = murl;
    	}).catch(function () {
        	// Handle if any request fails
			button.innerHTML = 'Install and Activate';
			button.disabled = false;
			window.location.href = murl;
    	});
